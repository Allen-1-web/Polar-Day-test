/**
 * Полярный день — мини-магазин без сервера.
...
function updateUser(email, patch) {
  ...
  users[email] = { ...current, ...patch, updatedAt: new Date().toISOString() };
  saveUsers(users);
  return true;
}
...
function setActiveNav(name) {
  document.querySelectorAll("[data-screen]").forEach((btn) => {
    const isActive = btn.getAttribute("data-screen") === name;
...
function renderAccount() {
  ...
  // Если пользователь уже в сессии, показываем профиль и прячем регистрацию
...
document.getElementById("form-register").addEventListener("submit", (e) => {
  ...
  users[email] = {
    password,
    displayName: computeDefaultDisplayName(email),
